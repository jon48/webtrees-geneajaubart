 ...
 *
 * webtrees-MyArtJaub
 * Copyright (C) 2024 Jonathan Jaubart
 *
 * Based on webtrees: online genealogy
 * Copyright (C) 2009-2024 webtrees development team
 *
...
(function (majEntryHelper, translations) {
  ...
  const setTypeAheadValue = (elementId, value) => {
     ...
     *
    * @param {string} elementId
    * @returns {{citationPageId: string|null, citationActCityId: string|null, citationActeFileId: string|null}}
     */
  const getSourceCitationsElementIds = elementId => {
    const citationParent = $('#' + elementId + '-ts-label');
    const citationDetailId = citationParent.children('button[data-bs-toggle]').first().data('bsTarget') || null;
    const citationActId = $(citationDetailId).find(".certificate-edit-group input[type='hidden']").first().attr('id') || null;

    return {
      citationPageId: citationDetailId.replace('collapse-', '') || null,
