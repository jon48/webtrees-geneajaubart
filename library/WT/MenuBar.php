...
class WT_MenuBar {
	...
	public static function getListsMenu() {
		...
		if (!$SEARCH_SPIDER) {
			...
			if(array_key_exists('perso_certificates', WT_Module::getActiveModules())) $menulist[]=new WT_Menu(WT_I18N::translate('Certificates'), 'module.php?mod=perso_certificates&mod_action=certificatelist&ged=' . WT_GEDURL, 'menu-perso-list-certificate');
			if(array_key_exists('perso_patronymiclineage', WT_Module::getActiveModules())) $menulist[]=new WT_Menu(WT_I18N::translate('Patronymic Lineages'), 'module.php?mod=perso_patronymiclineage&mod_action=patronymiclineage&ged=' . WT_GEDURL. $surname_url, 'menu-perso-list-lineage');
			//END PERSO
			if ($row->obje) {
