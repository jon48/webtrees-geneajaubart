...
* Copyright (C) 2011 Daniel Faivre.  All rights reserved.
*
...
*
* @package webtrees
* @version $Id: class_treeview.php Daniel Faivre $
*
*/
...
class TreeView {
	...
	*/
	function __construct($name='treeview') {
		$this->name = $name;
  ...
  * @param int $generations number of generations to draw
	*/
    ...
    $rootPerson = tvPerson::getInstance($rootPersonId);
    if (is_null($rootPerson))
      $rootPerson = new Person('');

    if (TV_SCRIPT_NAME == 'individual.php')
      $path = 'individual.php?pid='.$rootPerson->getXref().'&ged='.$GEDCOM.'&allPartners='.($this->allPartners ? "false" : "true").'#treeview';
    else
      $path = 'module.php?mod=treeview&mod_action=treeview&rootId='.$rootPerson->getXref().'&allPartners='.($this->allPartners ? "false" : "true");
    $r = '<a name="tv_content"></a><div id="'.$this->name.'_out" dir="ltr" class="tv_out">';
    
    // Read styles (20 maxi) in a hidden list
    $sd = TV_TREEVIEW_ROOT.'css/styles/';
    $rs = '<ul id="tvStylesSubmenu">';
    $cs = '<img src="'.$sd.'/button.gif" alt="d"  onclick="'.$this->name.'Handler.style(\''.$sd.'\', \'\', this);" title="'.tvI18N::translate('Default style').'" />';
    ...
    if (@is_dir($sd) && @is_readable($sd) && ($d=@opendir($sd))) {    	
    	while (($s = readdir($d)) !== false && ($nbStyles < 20)) {
    		if ($s[0] == '.' || !is_dir($sd.$s))
    			continue;
        $sHTML = '<img src="'.$sd.$s.'/button.gif" alt="'.$s[0].'"  onclick="'.$this->name.'Handler.style(\''.$sd.'\', \''.$s.'\', this);" title="'.tvI18N::translate('Style %s', $s).'" />';
        if ($s == $style) {
        	$cs = $sHTML;
    ...
    // open all boxes command : <li id="tvbOpen" class="tv_button"><img src="'.TV_OPEN_BOXES.'" alt="o" title="'.tvI18N::translate('Open visible boxes').'" /></li> is NOT enabled since it fire too much ajax requests. Need to be partially rewrited to do one ajax request only.
    // Add the toolbar
    $r .= '<div id="tv_tools"><ul>
<li id="tvToolsHandler" title="'.tvI18N::translate('You can drag the toolbox and double-click to change orientation').'"></li>
<li id="tvbZoomIn" class="tv_button"><img src="'.TV_ZOOMIN.'" alt="z+" title="'.tvI18N::translate('Zoom in').'" /></li>
<li id="tvbZoomOut" class="tv_button"><img src="'.TV_ZOOMOUT.'" alt="z-" title="'.tvI18N::translate('Zoom out').'" /></li>
<li id="tvbNoZoom" class="tv_button"><img src="'.TV_NOZOOM.'" alt="z0" title="'.tvI18N::translate('No zoom').'" /></li>
<li id="tvbLeft" class="tv_button"><img src="'.TV_LEFT.'" alt="|<" title="'.tvI18N::translate('Align left').'" /></li>
<li id="tvbCenter" class="tv_button"><img src="'.TV_CENTER.'" alt="<>" title="'.tvI18N::translate('Center on root person').'" /></li>
<li id="tvbRight" class="tv_button"><img src="'.TV_RIGHT.'" alt=">|" title="'.tvI18N::translate('Align right').'" /></li>
<li id="tvbDates" class="tv_button"><img src="'.TV_DATES.'" alt="d" title="'.tvI18N::translate('Hide / show dates').'" /></li>
<li id="tvbCompact" class="tv_button"><img src="'.TV_COMPACT.'" alt="c" title="'.tvI18N::translate('Compact tree / fixed width boxes').'" /></li>

<li id="tvbClose" class="tv_button"><img src="'.TV_CLOSE_BOXES.'" alt="f" title="'.tvI18N::translate('Close all details boxes').'" /></li>
<li id="tvStyleButton" class="tv_button">'.$cs.'</li>
<li id="tvbPrint" class="tv_button"><img src="'.TV_PRINT.'" alt="p" title="'.tvI18N::translate('Load full resolution medias and open print dialog').'" /></li>
<li class="tv_button'.($this->allPartners ? ' tvPressed' : '').'"><a href="'.$path.'" title="'.tvI18N::translate('Show or hide multiple life partners').'"><img src="'.TV_SFAMILY.'" alt="" /></a></li>';
    if (safe_GET('mod_action') != 'treeview')
      $r .=  '<li class="tv_button"><a href="module.php?mod=treeview&mod_action=treeview&rootId='.$rootPerson->getXref().'#tv_content" title="'.tvI18N::translate('View this tree in the full page interactive tree').'"><img src="'.TV_TREE.'" alt="t" /></a></li>'; 
    // Help, and hidden loading image
		$r .= '<li class="tv_button">'.help_link("TV_MODULE", "treeview").'</li>
  <li class="tv_button" id="'.$this->name.'_loading"><img src="'.TV_LOADING.'" alt="Loading..." /></li>
</ul>'.$rs;
      ...
      switch($firstLetter) {
        case 'c':
        	...
        	foreach($fidlist as $fid)
        		$flist[] = tvFamily::getInstance($fid);
  ...
  */
  public function getDetails($pid) {
    ...
    $person = tvPerson::getInstance($pid);
  ...
  */
  private function getPersonDetails(&$personGroup, &$person, $family) {
    ...
    $r .= '<a class="tv_link" href="'.$person->getHtmlUrl().'" title="'.tvI18N::translate('Display').' '.htmlentities(utf8_decode($person->getFullName())).'">'.PrintReady($person->getFullName()).'</a> <a href="module.php?mod=treeview&mod_action=treeview&allPartners='.($this->allPartners ? 'true' : 'false').'&rootId='.$person->getXref().'" title="'.tvI18N::translate('Display tree for').' '.htmlentities(utf8_decode($person->getFullName())).'"><img src="'.TV_TREE.'" class="tv_link tv_treelink" /></a>';
  ...
  private function drawPerson(&$person, $gen, $state=0, &$pfamily, $order, $isRoot=false) {
    ...
    if ($isRoot)
    	$r = '<table id="tvTreeBorder" class="tv_tree"><tbody><tr><td id="tv_tree_topleft"></td><td id="tv_tree_top"><div>'.
    		tvI18N::translate('Genealogical tree of ').$person->getFullName().
  ...
  */
  private function drawPersonName(&$p) {
  	if ($this->allPartners) {
    	...
    	$title = $title ? ' title="'.htmlentities(utf8_decode(tvI18N::translate('Child of %s', $title))).'"' : '';
  	}
  	else
  		$title = '';
	$sex = $p->getSex();
  ...
  */
  private function getThumbnail($personGroup, &$person) {
    global $MULTI_MEDIA, $SHOW_HIGHLIGHT_IMAGES, $TEXT_DIRECTION, $USE_MEDIA_VIEWER, $USE_SILHOUETTE, $GEDCOM;
    ...
    if ($MULTI_MEDIA && $SHOW_HIGHLIGHT_IMAGES) {
      ...
      if (!empty($object)) {
        ...
        $thumbnail .= "<img src=\"".TV_SCRIPT_FULL_PATH.$whichFile."\" vspace=\"0\" hspace=\"0\" class=\"$class\" alt=\"$mid\" title=\"\"";
    ...
    if (empty($thumbnail) && ($USE_SILHOUETTE)) {
      ...
      $thumbnail = "<img src=\"";
      if ($sex == 'F') {
        $thumbnail .= TV_DEFAULT_IMAGE_F;
      }
      else if ($sex == 'M') {
        $thumbnail .= TV_DEFAULT_IMAGE_M;
      }
      else {
        $thumbnail .= TV_DEFAULT_IMAGE_U;
      }
