 ...
 * @package webtrees
 * @subpackage Lists
 * @version $Id: indilist.php 11603 2011-05-25 08:15:52Z greg $
...
if (!$SEARCH_SPIDER) {
	if ($show_all=='yes') {
		...
		$list[]='<a href="indilist.php?show_all=yes'.'&amp;ged='.WT_GEDURL.'">'.WT_I18N::translate('All').'</a>';
	}
}
...
if ($showList) {
	$surns=WT_Query_Name::surnames($surname, $alpha, $SHOW_MARRIED_NAMES, false, WT_GED_ID);
	if ($surname_sublist=='yes') {
		// Show the surname list
		switch ($SURNAME_LIST_STYLE) {
		case 'style1';
			echo format_surname_list($surns, 3, true, 'indilist');
			break;
		case 'style3':
		...
		if ($count<$SUBLIST_TRIGGER_I) {
			...
			if (($surname || $show_all=='yes') && $count>$SUBLIST_TRIGGER_I) {
				...
				if ($show_all=='no') {
					...
					echo WT_I18N::translate('Individuals with surname %s', check_NN($surname));
					echo '</h2>';
				}
				echo '<div class="alpha_index"><p class="center">';
		...
		if ($showList) {
			...
			$indis=WT_Query_Name::individuals($surname, $alpha, $falpha, $SHOW_MARRIED_NAMES, false, WT_GED_ID);
