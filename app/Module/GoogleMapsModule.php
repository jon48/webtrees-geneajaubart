...
class GoogleMapsModule extends AbstractModule implements ModuleConfigInterface, ModuleTabInterface, ModuleChartInterface
{
    ...
    private function flags()
    {
        ...
        if ($action == 'ChangeFlag' && Filter::post('FLAGS') !== null) {
            ...
            <?php if (Filter::post('selcountry') == 'Countries') { ?>
                    ...
                    window.opener.document.getElementById('flagsDiv').innerHTML = "<img class=\"flag_gm_30\" src=\"<?php echo WT_STATIC_URL . WT_MODULES_DIR ?>googlemap/places/flags/<?php echo $country[Filter::post('FLAGS')] ?>.png\">&nbsp;&nbsp;<a href=\"#\" onclick=\"change_icon();return false;\"><?php echo I18N::translate('Change flag') ?></a>&nbsp;&nbsp;<a href=\"#\" onclick=\"remove_icon();return false;\"><?php echo I18N::translate('Remove flag') ?></a>";
        <?php } elseif (Filter::post('selstate') != "States") { ?>
                    ...
                    window.opener.document.getElementById('flagsDiv').innerHTML = "<img class=\"flag_gm_30\" src=\"<?php echo WT_STATIC_URL . WT_MODULES_DIR ?>googlemap/places/<?php echo $countrySelected . "/flags/" . Filter::post('selstate') . '/' . $flags_s[Filter::post('FLAGS')] ?>.png\">&nbsp;&nbsp;<a href=\"#\" onclick=\"change_icon();return false;\"><?php echo I18N::translate('Change flag') ?></a>&nbsp;&nbsp;<a href=\"#\" onclick=\"remove_icon();return false;\"><?php echo I18N::translate('Remove flag') ?></a>";
        <?php } else { ?>
                    window.opener.document.editplaces.icon.value = "places/<?php echo $countrySelected . '/flags/' . $flags[Filter::post('FLAGS')] ?>.png";
                    window.opener.document.getElementById('flagsDiv').innerHTML = "<img class=\"flag_gm_30\" src=\"<?php echo WT_STATIC_URL . WT_MODULES_DIR ?>googlemap/places/<?php echo $countrySelected . '/flags/' . $flags[Filter::post('FLAGS')] ?>.png\">&nbsp;&nbsp;<a href=\"#\" onclick=\"change_icon();return false;\"><?php echo I18N::translate('Change flag') ?></a>&nbsp;&nbsp;<a href=\"#\" onclick=\"remove_icon();return false;\"><?php echo I18N::translate('Remove flag') ?></a>";
        <?php } ?>
    ...
    private function getPlaceData(Fact $fact)
    {
        ...
        if ($has_latitude && $has_longitude) {
            ...
            if ($place_location && $place_location->pl_lati && $place_location->pl_long) {
                ...
                //PERSO Resize flag
                if (!empty($place_location->pl_icon)) { 
                    $icon_image = WT_STATIC_URL.WT_MODULES_DIR . 'googlemap/' . $place_location->pl_icon;	
                    list($flag_width, $flag_height) = mw\Functions\Functions::getResizedImageSize($icon_image, 25);
			
                    $result['mapdata']['width'] = $flag_width && $flag_width > 0 ? $flag_width : 0;
                    $result['mapdata']['height'] = $flag_height && $flag_height > 0 ? $flag_height : 0;
                }
                //END PERSO
            }
        }
            ...
            <script>
                ...
                function loadMap() {
                    ...
                    jQuery.each(locations, function(index, location) {
                        ...
                        //PERSO Resize flag
                        createMarker(point, html, location.tooltip, location.sv_lati, location.sv_long, location.sv_bearing, location.sv_elevation, location.sv_zoom, sv_point, location.pl_icon, location.width, location.height);
                        //END PERSO
    ...
    private function printGoogleMapMarkers($place2, $level, $parent, $levelm, $linklevels)
    {
        ...
        if (!$place2['lati'] || !$place2['long']) {
            ...
            $this->printHowManyPeople($level + 1, $parent);
            //PERSO Resize flags
            echo '</div>", "', $icon_image, '", "', Filter::escapeJs($place2['place']), '", "', $flag_width, '", "', $flag_height, '");';
            //END PERSO
        }
    }
            ...
            function loadMap(zoom, mapType) {
                ...
                if ($place_icon != '') {
                    ...
                    echo 'var image = {
                        ...
                        , "scaledSize" : new google.maps.Size(' . $flag_width . ',' . $flag_height . ')  // Image scaled size
                    };';
                    //END PERSO
            ...
            <tr>
                ...
                <td class="optionbox" colspan="2">
                    <div id="flagsDiv">
                        <?php if ($place_icon) { ?>
                        <!-- PERSO Resize flags -->
                        <img class="flag_gm_30" alt="<?php echo /* I18N: The emblem of a country or region */ I18N::translate('Flag') ?>" src="<?php echo WT_STATIC_URL, WT_MODULES_DIR, 'googlemap/', $place_icon ?>">
                        <!-- END PERSO -->
